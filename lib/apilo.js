...
export async function fetchAllNewOrders(lastSyncDate = null, maxOrders = 500) {
  ...
  while (hasMore) {
    let url = `/rest/api/orders/?limit=${limit}&offset=${offset}&sort=orderedAtDesc`;

    // If we have a last sync date, only fetch orders updated after that
    if (lastSyncDate) {
      const dateParam = encodeURIComponent(lastSyncDate);
      url += `&updatedAfter=${dateParam}`;
    }
    ...
    if (orders.length === 0) {
      ...
      allOrders = allOrders.concat(orders.map(order => mapOrderToDTO(order)));
      offset += limit;
